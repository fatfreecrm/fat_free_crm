...
module FatFreeCrm
  describe CustomFieldPair do
    ...
    it "should respond to pair" do
      expect(FatFreeCrm::CustomFieldPair.new).to respond_to(:pair)
    end
    ...
    describe "create_pair" do
      ...
      it "should create the pair" do
        ...
        expect(FatFreeCrm::CustomFieldPair.create_pair(@params)).to eq([foo1, foo2])
      end
    end
    ...
    describe "update_pair" do
      ...
      it "should update the pair" do
        ...
        expect(FatFreeCrm::CustomFieldPair).to receive(:find).with('3').and_return(foo1)

        expect(FatFreeCrm::CustomFieldPair.update_pair(@params)).to eq([foo1, foo2])
      end
    end

    describe "paired_with" do
      before(:each) do
        @field1 = FatFreeCrm::CustomFieldDatePair.new(name: 'cf_event_from')
        @field2 = FatFreeCrm::CustomFieldDatePair.new(name: 'cf_event_to')
      end
      ...
      it "should return the 1st field" do
        ...
        expect(FatFreeCrm::CustomFieldPair).to receive(:where).with(pair_id: 1).and_return([@field1])
