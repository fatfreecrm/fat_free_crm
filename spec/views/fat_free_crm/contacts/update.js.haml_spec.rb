  ...
  describe "/fat_free_crm/contacts/update" do
    ...
    describe "no errors:" do
      ...
      describe "on contacts index page -" do
        ...
        it "should replace [Edit Contact] with contact partial and highlight it" do
          ...
          render
          expect(rendered).to include("$('#edit_contact_#{@contact.id}').replaceWith('<li class=\\'brief highlight contact\\' id=\\'contact_#{@contact.id}\\'")
          expect(rendered).to include(%/$('#edit_contact_#{@contact.id}').effect("highlight"/)
        end
    ...
    describe "validation errors:" do
      ...
      describe "on contacts index page -" do
        ...
        it "should redraw the [edit_contact] form and shake it" do
          render
          expect(rendered).to include("$('#edit_contact_#{@contact.id}').html")
          expect(rendered).to include('crm.create_or_select_account(false)')
          expect(rendered).to include(%/$('#edit_contact_#{@contact.id}').effect("shake"/)
          expect(rendered).to include('focus()')
        end
      end
