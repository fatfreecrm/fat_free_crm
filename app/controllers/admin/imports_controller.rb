...
class Admin::ImportsController < Admin::ApplicationController
  ...
  def import
    ...
    Contact.transaction do
      CSV.foreach(params[:imports][:attached_file].tempfile.path, {:col_sep => ',', :headers => :first_row, :header_converters => :symbol}) do |row|
        ...
        when "City East", "City West"
          contact.cf_weekly_emails << row[:campus] unless contact.cf_weekly_emails.include?(row[:campus])
        ...
        if contact.save
          ...
          contact.errors.each {|attr, err| @imports.errors.add attr, "#{contact.first_name} #{contact.last_name}: #{err}"} 
        end
    ...
    if @imports.errors.empty?
      flash[:notice] = "Imported #{contacts_imported} contacts"
    else
    ...
    Contact.transaction do
      CSV.foreach(params[:imports][:attached_supporters_file].tempfile.path, {:col_sep => ',', :headers => :first_row, :header_converters => :symbol}) do |row|
        ...
        contact_hash = Hash.new.tap do |h|
          ...
          h[:phone] += "#{row[:work_phone]} [work]" unless row[:work_phone].blank?
        ...
        contact.assigned_to = User.find_by_first_name("geoff").id if contact.assigned_to.nil?
